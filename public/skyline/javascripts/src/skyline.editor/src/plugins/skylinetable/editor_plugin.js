...
(function() {
	...
	tinymce.create('Skyline.Editor.plugins.SkylineTable', {
		init : function(ed, url) {
			...
			// Register buttons
			each([
				['table', 'skyline_table.desc', 'mceInsertTable']
			], function(c) {
			...
			ed.onInit.add(function() {
				if (ed && ed.plugins.skylinecontextmenu) {
					ed.plugins.skylinecontextmenu.registerMenu(function(m,e,se,col,ed) {
						var sm;

						if (ed.dom.getParent(e, 'td') || ed.dom.getParent(e, 'th')) {
						  
							m.add({title : 'skyline_table.del', icon : 'delete_table', cmd : 'mceTableDelete', ui : true});
							m.addSeparator();

							// Row menu
							sm = m.addMenu({title : 'skyline_table.row'});
							sm.add({title : 'skyline_table.row_before_desc', icon : 'row_before', cmd : 'mceTableInsertRowBefore'});
							sm.add({title : 'skyline_table.row_after_desc', icon : 'row_after', cmd : 'mceTableInsertRowAfter'});
							sm.add({title : 'skyline_table.delete_row_desc', icon : 'delete_row', cmd : 'mceTableDeleteRow'});
							sm.addSeparator();
							sm.add({title : 'skyline_table.cut_row_desc', icon : 'cut', cmd : 'mceTableCutRow'});
							sm.add({title : 'skyline_table.copy_row_desc', icon : 'copy', cmd : 'mceTableCopyRow'});
							sm.add({title : 'skyline_table.paste_row_before_desc', icon : 'paste', cmd : 'mceTablePasteRowBefore'});
							sm.add({title : 'skyline_table.paste_row_after_desc', icon : 'paste', cmd : 'mceTablePasteRowAfter'});

							// Column menu
							sm = m.addMenu({title : 'skyline_table.col'});
							sm.add({title : 'skyline_table.col_before_desc', icon : 'col_before', cmd : 'mceTableInsertColBefore'});
							sm.add({title : 'skyline_table.col_after_desc', icon : 'col_after', cmd : 'mceTableInsertColAfter'});
							sm.add({title : 'skyline_table.delete_col_desc', icon : 'delete_col', cmd : 'mceTableDeleteCol'});
							return true;
						} else {
						  return false;
						}
		...
		_doExecCommand : function(command, user_interface, value) {
          ...
          inst.execCommand('mceBeginUndoLevel');
        	...
        	if (inst.settings.fix_table_elements) {
        		...
        		inst.dom.setOuterHTML(inst.dom.select('._mce_marker')[0], html);
          ...
          inst.selection.select(inst.dom.select("td",el)[0]);
          inst.selection.collapse(true);
	...
	// Register plugin
	tinymce.PluginManager.add('skylinetable', Skyline.Editor.plugins.SkylineTable);
})();