  ...
  def up
    ...
    Skyline::MediaFile.all.each do |file|
      ...
      if File.exist?(old_file_path)
        ...
        new_file_path = File.join(assets_path, "tmp", [new_file_key[0,2], new_file_key[2,2]].join('/'), file.id.to_s)
    ...
    Skyline::MediaFile.all.each do |file|
      ...
      old_file_path = File.join(assets_path, "tmp", [old_file_key[0,2], old_file_key[2,2]].join('/'), file.id.to_s)
