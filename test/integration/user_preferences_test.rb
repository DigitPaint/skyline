...
class UserPreferencesTest < ActionController::IntegrationTest
  ...
  context "User preferences" do
    setup do
      assert @skyline_path = Skyline::Engine.config.skyline.mounted_engine_path, "Skyline path couldn't be found, is Skyline mounted in routes.rb?"
    ...
    should "be able to be set through controller" do
      @u.post "#{@skyline_path}/user_preferences",
